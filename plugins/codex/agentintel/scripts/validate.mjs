...
const bundlePath = resolve(root, "dist/agentintel-mcp.mjs");
const expectedTools = [
...
if (manifest.name !== "agentintel")
  throw new Error("Codex plugin name drifted");
if (manifest.version !== packageJson.version)
  throw new Error("Version drifted");
if (!manifest.skills || !manifest.mcpServers)
  throw new Error("Skill or MCP missing");
const server = mcp.mcpServers?.["agentintel"];
if (
  server?.command !== "node" ||
  JSON.stringify(server.args) !==
    JSON.stringify(["./dist/agentintel-mcp.mjs"]) ||
  server.cwd !== "."
) {
