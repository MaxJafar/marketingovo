...
description: Use AGENTintel for cited competitive, creator, company, market, sales, or workforce research. Trigger when the user asks to compare organizations or creators, investigate a company, explain monitored social changes, search committed public evidence, or inspect research health.
---

# AGENTintel researcher

Use AGENTintel as an evidence and uncertainty system. The local service owns
...
- Use `agentintel_research_start` for an explicit research question about one
  or more named entities.
- Use `agentintel_compare_start` when the user wants the same metrics compared
  across two to fifty brands, companies or creators.
- Use `agentintel_run_get` when a run id already exists. Do not create a
  duplicate run merely because the previous run is still executing.
- Use `agentintel_search` to search committed local evidence without causing
  collection.
- Use `agentintel_entity_get` to inspect an entity and its observed
  identifiers. Do not treat a candidate identity match as confirmed.
- Use `agentintel_monitoring_status` to explain daemon, worker, queue or
...
2. Start the smallest useful run. Preserve the run id and poll
   `agentintel_run_get` until `succeeded`, `partial`, `failed` or `cancelled`.
...
sources are valid. Do not ask AGENTintel to bypass authentication, CAPTCHAs,
