...
def load_worker_pb2() -> ModuleType:
    ...
    try:
        from golem.intel.v1 import worker_pb2  # type: ignore[import-not-found]

        return worker_pb2
    except ModuleNotFoundError:
        repository_root = Path(__file__).resolve().parents[4]
        generated_root = Path(
            os.environ.get("GOLEM_INTEL_GEN_PYTHON", repository_root / "gen/python")
        ).resolve()
        if not (generated_root / "golem/intel/v1/worker_pb2.py").is_file():
            raise WorkerError(
                "protocol_bindings_unavailable",
                "Generated Python worker bindings are unavailable; run `buf generate`.",
            ) from None
        sys.path.insert(0, str(generated_root))
        from golem.intel.v1 import worker_pb2  # type: ignore[import-not-found,no-redef]
