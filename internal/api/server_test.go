...
func TestBearerAndOneTimeBrowserSessionBoundaries(t *testing.T) {
	...
	wrongOrigin := postJSON(t, browser, environment.server.URL+"/v1/comparisons", comparisonBody(), map[string]string{"Origin": "http://127.0.0.1:1", "X-Golem-CSRF": session.CSRF})
	...
	accepted := postJSON(t, browser, environment.server.URL+"/v1/comparisons", comparisonBody(), map[string]string{"Origin": environment.server.URL, "X-Golem-CSRF": session.CSRF})
...
func newTestAPI(t *testing.T) testAPIEnvironment {
	...
	if err := os.WriteFile(filepath.Join(dashboard, "index.html"), []byte("<!doctype html><title>Golem Intel</title>"), 0o600); err != nil {
		t.Fatal(err)
	}
