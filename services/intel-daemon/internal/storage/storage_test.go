...
func TestDurableRunCancellationReplayAndRecovery(t *testing.T) {
	...
	run, err := store.createRun(ctx, "demo", domain.WorkflowCompare, request, nil, domain.InputSnapshot{
		...
		SchemaID: "marketingovo.fixture-observations.v1", SizeBytes: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompleteRunPersistsArtifactsEntitiesAndSearch(t *testing.T) {
	...
	report := domain.Artifact{ID: "artifact-report", RunID: run.ID, Kind: "report", RelativePath: "runs/x/evidence/report.json", MediaType: "application/json", SHA256: string(make([]byte, 64)), SchemaID: "marketingovo.comparison-report.v1", DataClass: "public"}
