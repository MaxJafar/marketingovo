...
async function verifyWindows() {
  const expectedThumbprint =
    process.env.MARKETINGOVO_WINDOWS_CERTIFICATE_THUMBPRINT?.replaceAll(
      /\s/gu,
      "",
    ).toUpperCase();
  if (!expectedThumbprint || !/^[0-9A-F]{40}$/u.test(expectedThumbprint)) {
    throw new Error(
      "MARKETINGOVO_WINDOWS_CERTIFICATE_THUMBPRINT is required for Authenticode verification",
    );
  }
...
$signature = Get-AuthenticodeSignature -LiteralPath $env:MARKETINGOVO_VERIFY_ARTIFACT
...
if ($signature.SignerCertificate.Thumbprint -ne $env:MARKETINGOVO_EXPECTED_THUMBPRINT) { throw 'Authenticode signer thumbprint does not match the imported release certificate' }
  ...
  const verified = installers.map(({ path }) => {
    const output = run(
      ...
      ["-NoProfile", "-NonInteractive", "-Command", script],
      {
        env: {
          ...process.env,
          MARKETINGOVO_VERIFY_ARTIFACT: resolve(bundleRoot, path),
          MARKETINGOVO_EXPECTED_THUMBPRINT: expectedThumbprint,
        },
      },
    );
