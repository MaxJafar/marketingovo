...
or set `MARKETINGOVO_AUTO_UPDATE=off`. The request contains only target,
...
`src-tauri/binaries/marketingovo-node-<target-triple>` and the bundled runtime tree under
...
the product-scoped `marketingovo-node` Tauri sidecar. The unique name prevents a
...
Set `MARKETINGOVO_GOOGLE_DESKTOP_CLIENT_ID` while assembling the runtime and use
...
```bash
MARKETINGOVO_GOOGLE_DESKTOP_CLIENT_ID=YOUR_PUBLIC_ID.apps.googleusercontent.com \
...
and service removal. When `MARKETINGOVO_UPGRADE_BASELINE_TAG` points to an older
...
`marketingovo service install --credential-broker /absolute/path/to/broker`. The
validated broker path is persisted as an argv element, while master passwords
are never serialized into service definitions. Use `marketingovo service status`
or `marketingovo service uninstall` for lifecycle control.
