...
fn activate_existing_instance(handle: &tauri::AppHandle, arguments: &[String]) {
    ...
    let created = match ensure_startup_window(handle) {
        Ok(created) => created,
        Err(error) => {
            eprintln!("Golem SEO launcher: {error}");
            return;
        }
    };
    ...
    tauri::async_runtime::spawn(async move {
        ...
        .await
        {
            ...
            set_startup_status(
                &activation_handle,
                "The existing local service could not issue a new dashboard session. Run `golem-seo doctor` for diagnostics.",
                "Golem SEO — Local service unavailable",
            );
            activation_handle
                .state::<DashboardLaunchInFlight>()
                .0
...
async fn check_and_install_signed_update(handle: &tauri::AppHandle) -> Result<(), String> {
    set_startup_status(
        ...
        "Golem SEO — Checking for updates",
    );
    let updater = handle
        .updater_builder()
    ...
    let Some(update) = updater
        ...
        set_startup_status(
            handle,
            "Golem SEO is up to date. Starting the local service…",
            "Golem SEO",
        );
        return Ok(());
    };
    ...
    set_startup_status(
        handle,
        &format!("Downloading signed Golem SEO {version}…"),
        "Golem SEO — Installing update",
    );
        ...
        .download_and_install(
            move |chunk_size, total| {
                ...
                let progress = total
                    ...
                    .map(|total| {
                        format!(
                            "Downloading signed Golem SEO {progress_version}… {}%",
                            downloaded.saturating_mul(100) / total
                        )
                    })
                    .unwrap_or_else(|| {
                        format!(
                            "Downloading signed Golem SEO {progress_version}… {} MiB",
                            downloaded / (1024 * 1024)
                        )
                    });
                set_startup_status(&progress_handle, &progress, "Golem SEO — Installing update");
            },
            move || {
                set_startup_status(
                    &finished_handle,
                    "Download complete. Verifying the release signature…",
                    "Golem SEO — Verifying update",
                );
            },
        )
        .await
    ...
    set_startup_status(
        handle,
        &format!("Golem SEO {version} is installed. Restarting…"),
        "Golem SEO — Restarting",
    );
    handle.restart();
}
...
async fn run_packaged_daemon(
        ...
        match event {
            CommandEvent::Stdout(bytes) => {
                for raw_url in parser.push(&bytes) {
                    ...
                    if opened {
                        if let Some(window) = handle.get_webview_window("main") {
                            let _ = window.set_title("Golem SEO");
                        }
                        handle
                            .state::<DashboardReady>()
                            .0
    ...
    if open_dashboard && !window_opened {
        ...
        set_startup_status(
            &handle,
            "The local service stopped before the dashboard became ready. Run `golem-seo doctor` for diagnostics.",
            "Golem SEO — Local service unavailable",
        );
    }
    Ok(())
...
pub fn run() {
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if !entry.is_file() {
                return Err("bundled Golem SEO runtime is missing".into());
            }
            if !broker.is_file() {
            ...
            tauri::async_runtime::spawn(async move {
                if check_updates {
                    if let Err(error) = check_and_install_signed_update(&handle).await {
                        eprintln!("Golem SEO updater: {error}");
                        set_startup_status(
                            &handle,
                ...
                .await
                {
                    ...
                    set_startup_status(
                        &handle,
                        "The packaged local service could not start. Run `golem-seo doctor` for diagnostics.",
                        "Golem SEO — Local service unavailable",
                    );
                }
            });
        ...
        .expect("Golem SEO desktop runtime failed");
...
mod tests {
    ...
    fn parses_a_dashboard_line_split_across_stdout_chunks() {
        let mut parser = DashboardOutputParser::default();
        assert!(parser.push(b"Golem SEO\nDash").is_empty());
        assert_eq!(
            parser.push(
    ...
    fn signed_updates_run_once_before_a_release_instance_starts() {
        let normal = vec![OsString::from("golem-seo-desktop")];
        let disabled = vec![
            OsString::from("golem-seo-desktop"),
    ...
    fn a_second_background_launch_stays_headless_but_a_user_launch_activates() {
        assert!(foreground_launch_requested(&["golem-seo-desktop".into()]));
        assert!(!foreground_launch_requested(&[
            "golem-seo-desktop".into(),
            "--background".into(),
        ]));
    }
}
