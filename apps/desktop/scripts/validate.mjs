...
if (
  ...
  throw new Error(
    "the desktop webview capability must contain zero permissions",
  );
}
if (
  cargoSource.includes("tauri-plugin-shell") ||
  cargoSource.includes("tauri-plugin-fs")
) {
  throw new Error(
    "shell and filesystem plugins must not be linked into the desktop shell",
  );
}
if (
  librarySource.includes("invoke_handler") ||
  librarySource.includes("#[tauri::command]")
) {
  throw new Error(
    "the desktop shell must not register webview invoke commands",
  );
}
...
if (
  ...
  throw new Error(
    "desktop tickets must use stdin and packaged Python must bypass uv",
  );
}
if (
  ...
  throw new Error(
    "the bootstrap ticket must use a zeroized bounded stdin pipe",
  );
}
if (
  ...
  throw new Error(
    "the signed bundle must pin and seal a complete Python runtime",
  );
}
