...
Golem SEO is a local-first system with one daemon as the authority for jobs,
...
identities so this behavior survives a `.golemseo` transfer.
...
write, while audit events record only structural metadata. `.golemseo` transfer
...
browser. Import rebuilds the normalized table from sanitized `.golemseo` page
...
`@golem-seo/contracts`. MCP derives Zod validators from those schemas and
