# AGENTintel MCP bridge

This package projects exactly six high-level evidence workflows from the
generated AGENTintel contracts. It intentionally omits collectors, policy
mutation, credentials, contact reveal/export, deletion, outreach and employment
decisions.

Two transports share the same server factory:

- `agentintel-mcp` / `dist/stdio.js` for local editor and coding-agent clients;
- `agentintel-mcp-http` / `dist/http-cli.js` for authenticated Streamable HTTP.
